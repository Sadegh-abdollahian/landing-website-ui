...
switchThemeBtn.addEventListener("click", () => {
    ...
    if (document.documentElement.classList.contains("dark-theme")) {
        ...
        switchThemeBtn.innerHTML = lightThemeIcon;
    }
});
...
// adds the dark-theme class to html if user's theme is dark in local storage
...
// Custom fucntions
function removeActivaeClass(className) {
...
function navigationTabsInit(
    ...
    listItems.forEach((listItem) => {
        listItem.addEventListener("click", () => {
            removeActivaeClass(listItemActiveClass);
            removeActivaeClass(contentItemShowClass);
            listItem.classList.add(listItemActiveClass);
            let portfolioId = listItem.getAttribute("data-content-id");
            document
                .getElementById(portfolioId)
                .classList.add(contentItemShowClass);
        });
    });
}
...
menuItems.forEach((item) => {
    document.addEventListener("DOMContentLoaded", () => {
        item.addEventListener("click", (event) => {
            ...
            removeActivaeClass("menu__item--active");
            item.classList.add("menu__item--active");

            let sectionClass = item.getAttribute("data-section");
            let sectionOffset =
                document.querySelector(`.${sectionClass}`).offsetTop - 90;
